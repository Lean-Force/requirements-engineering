...
USM AI Chat の振る舞いを Gherkin で網羅的に記述したもの。用語は `MODEL.md` のユビキタス言語(アクター / ステップ / タスク / ストーリー)に揃えている。
...
| `activity.feature` | ステップ(ナラティブフロー上のステップ)と所属するタスク | domain |
| `action.feature` | タスク(Actor × Activity の交点)と配下ストーリーのカスケード | domain |
| `story.feature` | ストーリー(必ずタスクにぶら下がる) | domain |
