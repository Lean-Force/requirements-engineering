...
**論点(議論ポイント)**
- CRUD・解決時の結論(rationale)必須・要素削除時の孤児掃除・注入テキスト — L1(discussions)
- ルートのバリデーションと全配線 — L2 / UI の追加→解決→削除 — L3(discussion.feature)
- 未解決の論点を AI が踏まえて決め打ちしない — L4(論点ケース)

**PBI 化(EARS 記法)**
- EARS 5 パターンの分類器(受理・拒否) — L1(ears)
- ルートのバリデーション・ストーリー文脈の解決 — L2
- 知識の閾値が要求に正確に入る / 全行 EARS 適合 / 未解決論点は未決事項へ — L4(PBI ケース)
