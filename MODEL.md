...
| 概念 | 役割 | 例 |
|---|---|---|
| **Actor(アクター)** | 登場人物 / 利用者種別。ボードでは「行」と「色」で区別する。 | 店員、お客様、管理者 |
| **Activity(アクティビティ)** | ナラティブフロー上の1単位(場面/局面)。**順序を持つ**(並び順=左→右=時系列)。1つの Activity に複数アクターが参加しうる。 | 「商品の受け渡し」 |
| **Action(行動)** | **あるアクターが・ある Activity で**行う行動。バックボーンの付箋。`Actor × Activity` の交点。 | 店員が「商品を受け取る」/ お客様が「商品を渡す」 |
| **Story(ストーリー)** | ある行動(Action)を実現するユーザーストーリー。「◯◯は〜したい。なぜなら〜だからだ。」(目的・理由まで書く)。複数あれば時系列に並べる。 | 「店員は、クーポンを知らせたい。なぜなら、お得に買い物してもらい再来店につなげたいからだ。」 |
...
Action   = Actor(1) × Activity(1)   (どのアクターの・どの Activity の行動か)
Action ──< Story                 (時系列に並ぶ)
```
...
「Story / Actor / Activity」の3軸があり、それらを結ぶ交点(セル)が **Action** である。
...
- **activity line(バックボーン)**:行 = Actor(色分け)、列 = Activity(共有タイムライン)。
  セル(Actor × Activity)= Action の付箋(行動)。Activity は途中挿入も可(`addActivity(map, index)`)。
...
- **story line**:activity line 全体の下に置く帯。各 Activity 列の下に、その列の各 Action の Story を
  **アクター色で**まとめて並べる(同一 Action の複数 Story は時系列に横並び)。
  Story は引き続き Action に紐づく(`data-action-id` で activity line の行動と対応)。
