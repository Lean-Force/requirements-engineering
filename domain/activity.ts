...
export interface Activity {
  ...
  actions: Action[];
  /**
   * 時系列(ナラティブフロー)に属さないステップ = 随時・例外・定期。
   * true のステップは正規化でバックボーン末尾へまとめられ、UI では区切りの右に
   * 「随時」として表示される(タイムラインの意味を守る)。
   */
  standalone?: boolean;
  /**
   * バックボーン内のアクティビティ(連続するステップをまとめる帯)の名前(例: 受付 / 審査・承認 / 実行・通知)。
   * 同じ flowName のステップは正規化で隣接にまとめられ、UI では列群の上に
   * バンドとして表示される。随時(standalone)のステップには付かない。
   */
  flowName?: string;
  /**
